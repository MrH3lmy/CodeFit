...
public class DeckService {
    ...
    private static final JavaBackendCard[] JAVA_BACKEND_STARTER_CARDS = {
            ...
            new JavaBackendCard("Java BE 03 - JDBC & SQL", "Why should backend code prefer PreparedStatement over string-concatenated SQL?", "PreparedStatement binds parameters separately from SQL text, which helps prevent SQL injection and lets the driver handle type conversion.", CardType.CONCEPT, "PreparedStatement prevents SQL injection by binding parameters|It uses bind parameters instead of concatenating user input", ValidationMode.CASE_INSENSITIVE, null, "Mention parameter binding and SQL injection.", "SQL", null),
            new JavaBackendCard("Java BE 03 - JDBC & SQL", "users(id, email, created_at): write SQL to list the 5 newest user emails.", "SELECT email FROM users ORDER BY created_at DESC LIMIT 5;", CardType.SQL_QUERY, "SELECT email FROM users ORDER BY created_at DESC LIMIT 5;|SELECT email FROM users ORDER BY created_at DESC LIMIT 5", ValidationMode.NORMALIZED_SPACING, null, "Order newest first, then cap the result size.", "SQL", null),
            new JavaBackendCard("Java BE 03 - JDBC & SQL", "What is a database transaction?", "A transaction is a unit of work that should commit completely or roll back completely so related changes stay consistent.", CardType.CONCEPT, "unit of work that commits or rolls back|all-or-nothing unit of work", ValidationMode.CASE_INSENSITIVE, null, "Think ACID and all-or-nothing changes.", "SQL", null),
            ...
            new JavaBackendCard("Java BE 04 - Spring Boot REST APIs", "Why is constructor injection preferred for required Spring dependencies?", "Constructor injection makes dependencies explicit, supports final fields, and fails fast when a required bean is missing.", CardType.CONCEPT, "explicit dependencies final fields fail fast|required dependencies are explicit and immutable", ValidationMode.CASE_INSENSITIVE, null, "Think testability, immutability, and required collaborators.", "Spring REST", null),
            ...
            new JavaBackendCard("Java BE 07 - Security & Auth", "What is a key tradeoff between server-side sessions and JWTs?", "Sessions keep auth state on the server and are easy to revoke; JWTs are usually stateless for the server but need careful expiration and revocation design.", CardType.CONCEPT, "sessions are server-side and revocable, JWTs are stateless but harder to revoke|JWT stateless session server state", ValidationMode.CASE_INSENSITIVE, null, "Compare where auth state lives and how revocation works.", "Security", null),
            new JavaBackendCard("Java BE 08 - Build, Git & Deployment", "Maven command: run the test phase for this project.", "mvn test", CardType.COMMAND, "mvn test|./mvnw test", ValidationMode.COMMAND_NORMALIZED, "Runs unit tests and earlier lifecycle phases needed for test execution.", "Use the Maven Wrapper variant if the project includes mvnw.", "Deployment", 45),
            new JavaBackendCard("Java BE 08 - Build, Git & Deployment", "Maven command: clean previous build outputs and package the application artifact.", "mvn clean package", CardType.COMMAND, "mvn clean package|./mvnw clean package", ValidationMode.COMMAND_NORMALIZED, "Deletes target/ and builds the packaged artifact after running lifecycle phases up to package.", "clean removes generated outputs; package creates the jar or war.", "Deployment", 60),
            new JavaBackendCard("Java BE 08 - Build, Git & Deployment", "Spring Boot command-line option: start the app with the prod profile active.", "java -jar app.jar --spring.profiles.active=prod", CardType.COMMAND, "java -jar app.jar --spring.profiles.active=prod|SPRING_PROFILES_ACTIVE=prod java -jar app.jar", ValidationMode.COMMAND_NORMALIZED, "Application starts with prod profile-specific configuration enabled.", "Profiles select environment-specific beans and properties.", "Deployment", 75),
            new JavaBackendCard("Java BE 08 - Build, Git & Deployment", "Why should secrets and environment-specific settings live outside committed source code?", "External configuration lets each environment provide its own values and prevents committing credentials into version control.", CardType.CONCEPT, "prevents committing secrets and supports per-environment config|keeps credentials out of source control", ValidationMode.CASE_INSENSITIVE, null, "Think profiles, environment variables, and source control safety.", "Deployment", null),
            new JavaBackendCard("Java BE 04 - Spring Boot REST APIs", "Which Spring annotation centralizes exception handling across controllers?", "@ControllerAdvice", CardType.RECALL, "@ControllerAdvice|@RestControllerAdvice", ValidationMode.CASE_INSENSITIVE, null, null, "Spring REST", null)
    };
